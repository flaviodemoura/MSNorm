...
A constructive proof of the Modular Strong Normalisation Theorem is presented by S. Lengrand in %\cite{LengrandPhD}% and some of the basic notions used in this proof, such as strong normalisation, is already formalised in Coq %\footnote{\scriptsize \url{http://www.lix.polytechnique.fr/~lengrand/Work/HDR/Coq/First-order/NormalisationTheory.v}}%. The strong normalisation property is defined in %\cite{LengrandPhD}% %\fab{\sout{in terms of}}% %\fab{using}% a specialized inductive principle that should hold for all predicate, i.e. through a second order formula. 
Our proof of the Modular Strong Normalisation Theorem follows the ideas in %\fab{\sout{\cite{LengrandPhD}} Lengrand's formalization}%, but we use only the standard %\fab{(essa definiao é realmente "padrão"? não seria interessante uma referências para corroborar com a afirmação?)}% inductive definition of strong normalisation, proved to be equivalent to Lengrand's notion of termination. In this way, beside proving the theorem not present \fab{qual?} in the formalisation mentioned above, we understand that we achieved a simpler and easier to follow formalisation. 
