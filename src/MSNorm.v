...
(** 
    ...
    In his constructive theory of normalisation, Lengrand defines strong normalisation as a second-order property based on patriarchal predicates:
...
This definition is equivalent to the standard ones that can be find in the literature, such as the following definition which is based on the notion of normal forms:
...
(* end hide *)
Inductive SN_size {A} (n : nat) (R : Red A) (t : A) : Prop :=
 | SN_intro : (forall t', R t t' -> exists k, k < n /\ SN_size k R t') -> SN_size n R t.

Definition SNbound {A} (R : Red A ) (t : A) := exists n, SN_size n R t.
...
The main theorem of this formalisation is named [ModStrNorm], after modular strong normalisation theorem, and states that ...
