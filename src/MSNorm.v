...
On the other hand, the preservation of strong normalisation property (PSN) is known to be not satisfied for some explicit substitution calculi %\cite{Mellies95}%. Explicit substitution calculi %\cite{Lins86,ACCL91,kes09}% presents some formalisation for the substituion operation, defined as a meta-operation in the $\lambda$-calculus, and the PSN property guarantees that any strongly normalising term, i.e. terminating, in the $\lambda$-calculus is also strongly normalising in the explicit substitution calculus.       
...
A constructive proof of the Modular Strong Normalisation Theorem is presented by S. Lengrand in %\cite{LengrandPhD}% and some of the basic notions used in this proof, such as strong normalisation, is already formalised in Coq %\footnote{\scriptsize \url{http://www.lix.polytechnique.fr/~lengrand/Work/HDR/Coq/First-order/NormalisationTheory.v}}%. The strong normalisation property is defined in %\cite{LengrandPhD}% using a specialized inductive principle that should hold for all predicate, i.e. through a second order formula. 
Our proof of the Modular Strong Normalisation Theorem follows the ideas in Lengrand's formalization, but we use only the standard %\fab{(essa definiao é realmente "padrão"? não seria interessante uma referências para corroborar com a afirmação?)}% inductive definition of strong normalisation, proved to be equivalent to Lengrand's notion of termination. In this way, beside proving the theorem not present %\fab{qual?}% %\dan{'the' refere-se ao teorema citado anteriormente no texto}% in the formalisation mentioned above, we understand that we achieved a simpler and easier to follow formalisation. 
...
(**
In this section, we present the Modular Strong Normalisation Theorem whose formalisation will be detailed in the next section. This is an abstract theorem about termination of reduction relations through the well known simulation technique %\cite{BN98}%. In order to fix notation, let [A] and [B] be sets. A relation from [A] to [B] is a subset of $A\times B$. If $R$ is a relation from [A] to [B] then we write $aRb$ instead of $(a,b) \in R$, and in this case, we say that $a$ %{\it reduces}% to $b$. %\cdan{If one uses arrow notation to represent relations, and}{Using arrows to denote relations and}% $\to$ %\cdan{is}{being}% a relation from [A] to [B] then $\leftarrow$ %\cdan{is}{denotes}% the inverse relation, from [B] to [A]. If $\to_1$ is a relation from $A$ to $B$, and $\to_2$ is a relation from $B$ to $C$ then the composition of $\to_1$ with $\to_2$, written $\to_1\cdot \to_2$, is a relation from $A$ to $C$. A relation from a set to itself is a %{\it reduction relation \dan{over a set}}%, i.e. a reduction relation %\dan{over A}% is a subset of $A\times A$. If $\to_A$ is a reduction relation over $A$, then a %{\it reduction sequence}% is a %\odan{finite or infinite}% sequence of the form $a \to_A a_1 \to_A a_2 \to_A \ldots$ A %\odan{finite}% reduction $a \to_A a_1 \to_A a_2 \to_A \ldots \to_A a_n\ (n\geq 0)$ is a $n$-step reduction from $a$. %\dan{A reduction is finite if it is a $n$-step reduction for some $n \in \mathbb{N}$ and infinite otherwise.}% We write $\to_A^+$ (resp. $\to_A^*$) for the transitive (resp. reflexive transitive) closure of $\to_A$. *) 
...
(**
In order to present the %\cdan{Modular Strong Normalisation Theorem}{theorem}%, we need to define the notions of strong and weak simulation. In the following definitions $A$ and $B$ are arbitrary sets:

%\begin{definition}\label{def:sws}
Let $\to$ be a relation from $A$ to $B$, $\to_A$ be a reduction relation over $A$  and $\to_B$ be a reduction relation over $B$. The reduction relation $\to_B$ {\it strongly} (resp. {\it weakly}) simulates $\to_A$ through $\to$ if $(\leftarrow \cdot \to_A) \subseteq (\to_B^+ \cdot \leftarrow)$ (resp. $(\leftarrow \cdot \to_A) \subseteq (\to_B^* \cdot \leftarrow)$). \dan{seria interessante usar a mesma nota\c{c}\~ao utilizada na figura}
